...
# v3 here names the training recipe, not the promoted run ID. Keep the label
# outside the reserved v_ model namespace until a full run is actually blessed.
#
# This recipe stays on the current MLP projector path and enables the latest
# trainer-side scale fixes together:
# - composite checkpoint monitoring
# - direct norm matching (kept conservative because raw prompt norm MSE is large)
# - the new learned output calibrator
...
export RUN_LABEL="${RUN_LABEL:-full_mlp_calibrated_dnorm}"
...
export TRAIN_SDXL_PROMPT_NORM_MATCH_WEIGHT="${TRAIN_SDXL_PROMPT_NORM_MATCH_WEIGHT:-1e-5}"
export TRAIN_SDXL_POOLED_NORM_MATCH_WEIGHT="${TRAIN_SDXL_POOLED_NORM_MATCH_WEIGHT:-5e-4}"
